    ...
    `;t?n.innerHTML=`
      ...
      `:n.innerHTML=`
        ...
        <button id="hearly-dismiss-btn" style="
          ...
          flex-shrink: 0;
        ">×</button>
      `,document.body.appendChild(n),document.getElementById(`hearly-activate-btn`)?.addEventListener(`click`,()=>{chrome.runtime.sendMessage({type:`ACTIVATE_HEARLY`}),n.remove()}),document.getElementById(`hearly-train-btn`)?.addEventListener(`click`,()=>{chrome.runtime.sendMessage({type:`OPEN_POPUP`}),n.remove()}),document.getElementById(`hearly-dismiss-btn`)?.addEventListener(`click`,()=>{n.remove()})})}s()?(c(),l(),console.log(`Hearly: Zoom meeting detected`)):console.log(`Hearly: Zoom page, no active meeting`);var u=null,d=null,f=null,p=null;function m(e){p||(console.log(`[Hearly] Starting transcription recorder...`),chrome.storage.local.get(`hearly_app_settings`,t=>{let n=t.hearly_app_settings?.language??`en`;p=new a(e,`others`,(e,t)=>{(async()=>{let r=await i(e);r&&chrome.runtime.sendMessage({type:`HEARLY_TRANSCRIBE_CHUNK`,audioBase64:e,samples:r.samples,sampleRate:r.sampleRate,language:n,speaker:`others`,timestamp:t})})()}),p.start()}))}function h(){p&&=(p.stop(),null),console.log(`[Hearly] Transcription recorder stopped`)}function g(){h(),f&&=(f.disconnect(),null),u&&=(u.close(),null),d&&=(d.getTracks().forEach(e=>e.stop()),null),console.log(`[Hearly] Audio capture stopped on Zoom page`),chrome.runtime.sendMessage({type:`HEARLY_AUDIO_STOPPED`,platform:`zoom`})}chrome.runtime.onMessage.addListener((e,t,n)=>{if(e.type===`HEARBEAT_PING`)return n({status:`pong`}),!0;e.type===`HEARLY_START_AUDIO`&&e.streamId&&navigator.mediaDevices.getUserMedia({audio:{mandatory:{chromeMediaSource:`tab`,chromeMediaSourceId:e.streamId}},video:!1}).then(e=>{if(d=e,u=new AudioContext({sampleRate:16e3}),u.state===`suspended`){let e=()=>{u?.resume(),window.removeEventListener(`click`,e)};window.addEventListener(`click`,e)}f=u.createMediaStreamSource(e),f.connect(u.destination),console.log(`[Hearly] Audio capture started on Zoom page (tabCapture)`),chrome.runtime.sendMessage({type:`HEARLY_AUDIO_STARTED`,platform:`zoom`}),chrome.storage.local.get(`hearly_transcript`,t=>{t.hearly_transcript?.isEnabled&&m(e)})}).catch(e=>{console.warn(`[Hearly] Zoom tabCapture stream failed:`,e),chrome.runtime.sendMessage({type:`HEARLY_AUDIO_ERROR`,error:String(e)})}),e.type===`HEARLY_STOP_AUDIO`&&g(),e.type===`HEARLY_FILTER_STATE_CHANGED`&&chrome.storage.local.get(`hearly_filter`,e=>{window.postMessage({source:`hearly-content`,type:`FILTER_STATE_CHANGED`,active:e.hearly_filter?.isActive===!0},window.location.origin)}),e.type===`HEARLY_TRANSCRIPT_STATE_CHANGED`&&chrome.storage.local.get(`hearly_transcript`,e=>{let t=e.hearly_transcript?.isEnabled===!0;window.postMessage({source:`hearly-content`,type:`TRANSCRIPT_STATE_CHANGED`,enabled:t},window.location.origin),t?d&&!p&&m(d):h()})});