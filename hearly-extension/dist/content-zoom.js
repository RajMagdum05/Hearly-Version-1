function e(){let e=window.location.pathname;return window.location.hostname.includes(`zoom.us`)&&e.includes(`/wc/`)}function t(){chrome.runtime.sendMessage({type:`MEETING_DETECTED`,payload:{platform:`zoom`}})}function n(){document.getElementById(`hearly-banner`)||chrome.storage.local.get(`hearly_enrollment`,e=>{let t=e?.hearly_enrollment?.isEnrolled===!0,n=document.createElement(`div`);n.id=`hearly-banner`,n.style.cssText=`
      position: fixed;
      top: 16px;
      left: 50%;
    ...
    `;t?n.innerHTML=`
      ...
      `:n.innerHTML=`
        ...
        <button id="hearly-dismiss-btn" style="
          ...
          flex-shrink: 0;
        ">×</button>
      `,document.body.appendChild(n),document.getElementById(`hearly-activate-btn`)?.addEventListener(`click`,()=>{chrome.runtime.sendMessage({type:`ACTIVATE_HEARLY`}),n.remove()}),document.getElementById(`hearly-train-btn`)?.addEventListener(`click`,()=>{chrome.runtime.sendMessage({type:`OPEN_POPUP`}),n.remove()}),document.getElementById(`hearly-dismiss-btn`)?.addEventListener(`click`,()=>{n.remove()})})}e()?(t(),n(),console.log(`Hearly: Zoom meeting detected`)):console.log(`Hearly: Zoom page, no active meeting`);var r=null,i=null,a=null;function o(){a&&=(a.disconnect(),null),r&&=(r.close(),null),i&&=(i.getTracks().forEach(e=>e.stop()),null),console.log(`[Hearly] Audio capture stopped on Zoom page`),chrome.runtime.sendMessage({type:`HEARLY_AUDIO_STOPPED`,platform:`zoom`})}chrome.runtime.onMessage.addListener(e=>{e.type===`HEARLY_START_AUDIO`&&e.streamId&&navigator.mediaDevices.getUserMedia({audio:{mandatory:{chromeMediaSource:`tab`,chromeMediaSourceId:e.streamId}},video:!1}).then(e=>{i=e,r=new AudioContext({sampleRate:16e3}),a=r.createMediaStreamSource(e),console.log(`[Hearly] Audio capture started on Zoom page (tabCapture)`),chrome.runtime.sendMessage({type:`HEARLY_AUDIO_STARTED`,platform:`zoom`})}).catch(e=>{console.warn(`[Hearly] Zoom tabCapture stream failed:`,e),chrome.runtime.sendMessage({type:`HEARLY_AUDIO_ERROR`,error:String(e)})}),e.type===`HEARLY_STOP_AUDIO`&&o()});