    ...
    `;t?n.innerHTML=`
      ...
      `:n.innerHTML=`
        ...
        <button id="hearly-dismiss-btn" style="
          ...
          flex-shrink: 0;
        ">×</button>
      `;let i=()=>{x||document.getElementById(`hearly-banner`)||(document.body?(document.body.appendChild(n),document.getElementById(`hearly-activate-btn`)?.addEventListener(`click`,()=>{chrome.runtime.sendMessage({type:`ACTIVATE_HEARLY`}),x=!0,n.remove()}),document.getElementById(`hearly-train-btn`)?.addEventListener(`click`,()=>{chrome.runtime.sendMessage({type:`OPEN_POPUP`}),x=!0,n.remove()}),document.getElementById(`hearly-dismiss-btn`)?.addEventListener(`click`,()=>{x=!0,n.remove()})):new MutationObserver((e,t)=>{document.body&&(t.disconnect(),i())}).observe(document.documentElement,{childList:!0,subtree:!0}))};i()})}S(),y()?(b(),console.log(`Hearly: Zoom meeting detected`)):console.log(`Hearly: Zoom page, no active meeting`);var C=y(),w=new MutationObserver(()=>{let e=y();e!==C&&(e&&b(),C=e),!document.getElementById(`hearly-banner`)&&!x&&S()}),T=()=>{if(!document.body){setTimeout(T,50);return}w.observe(document.body,{subtree:!0,childList:!0})};T();var E=null,D=null,O=null,k=null;function A(e){k||(console.log(`[Hearly] Starting transcription recorder...`),k=new t(e,`others`,(e,t)=>{chrome.runtime.sendMessage({type:`HEARLY_TRANSCRIBE_CHUNK`,audioBase64:e,speaker:`others`,timestamp:t})}),k.start())}function j(){let e=k;e&&(e.stop(),k=null),console.log(`[Hearly] Transcription recorder stopped`)}function M(){j();let e=O;e&&(e.disconnect(),O=null);let t=E;t&&(t.close(),E=null);let n=D;n&&(n.getTracks().forEach(e=>e.stop()),D=null),console.log(`[Hearly] Audio capture stopped on Zoom page`),chrome.runtime.sendMessage({type:`HEARLY_AUDIO_STOPPED`,platform:`zoom`})}chrome.runtime.onMessage.addListener((e,t,n)=>{if(e.type===`HEARLY_NEW_TRANSCRIPT_ENTRY`&&e.entry&&g(e.entry),e.type===`HEARBEAT_PING`)return n({status:`pong`}),!0;e.type===`HEARLY_START_AUDIO`&&e.streamId&&navigator.mediaDevices.getUserMedia({audio:{mandatory:{chromeMediaSource:`tab`,chromeMediaSourceId:e.streamId}},video:!1}).then(e=>{if(D=e,E=new AudioContext({sampleRate:16e3}),E.state===`suspended`){let e=()=>{E?.resume(),window.removeEventListener(`click`,e)};window.addEventListener(`click`,e)}O=E.createMediaStreamSource(e),O.connect(E.destination),console.log(`[Hearly] Audio capture started on Zoom page (tabCapture)`),chrome.runtime.sendMessage({type:`HEARLY_AUDIO_STARTED`,platform:`zoom`}),chrome.storage.local.get(`hearly_transcript`,t=>{t.hearly_transcript?.isEnabled&&A(e)})}).catch(e=>{console.warn(`[Hearly] Zoom tabCapture stream failed:`,e),chrome.runtime.sendMessage({type:`HEARLY_AUDIO_ERROR`,error:String(e)})}),e.type===`HEARLY_STOP_AUDIO`&&M(),e.type===`HEARLY_FILTER_STATE_CHANGED`&&chrome.storage.local.get(`hearly_filter`,e=>{window.postMessage({source:`hearly-content`,type:`FILTER_STATE_CHANGED`,active:e.hearly_filter?.isActive===!0},window.location.origin)}),e.type===`HEARLY_TRANSCRIPT_STATE_CHANGED`&&chrome.storage.local.get(`hearly_transcript`,e=>{let t=e.hearly_transcript?.isEnabled===!0;window.postMessage({source:`hearly-content`,type:`TRANSCRIPT_STATE_CHANGED`,enabled:t},window.location.origin),t?D&&!k&&A(D):j()})});function N(){if(document.getElementById(`hearly-indicator`))return;let e=document.createElement(`div`);e.id=`hearly-indicator`,e.style.cssText=`
    position: fixed;
    bottom: 24px;
    right: 24px;
    z-index: 999999;
