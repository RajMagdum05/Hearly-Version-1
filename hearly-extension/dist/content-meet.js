    ...
    `;t?n.innerHTML=`
      ...
      `:n.innerHTML=`
        ...
        <button id="hearly-dismiss-btn" style="
          ...
          flex-shrink: 0;
        ">×</button>
      `;let i=()=>{S||document.getElementById(`hearly-banner`)||(document.body?(document.body.appendChild(n),document.getElementById(`hearly-activate-btn`)?.addEventListener(`click`,()=>{chrome.runtime.sendMessage({type:`ACTIVATE_HEARLY`}),S=!0,n.remove()}),document.getElementById(`hearly-train-btn`)?.addEventListener(`click`,()=>{chrome.runtime.sendMessage({type:`OPEN_POPUP`}),S=!0,n.remove()}),document.getElementById(`hearly-dismiss-btn`)?.addEventListener(`click`,()=>{S=!0,n.remove()})):new MutationObserver((e,t)=>{document.body&&(t.disconnect(),i())}).observe(document.documentElement,{childList:!0,subtree:!0}))};i()})}C(),y()?(b(),console.log(`Hearly: Meet room detected (lobby or active)`)):console.log(`Hearly: Meet page loaded, no active meeting`);var w=y();new MutationObserver(()=>{let e=y();e!==w&&(e?b():x(),w=e),!document.getElementById(`hearly-banner`)&&!S&&C()}).observe(document.querySelector(`title`)||document.documentElement,{subtree:!0,characterData:!0,childList:!0});var T=null,E=null,D=null,O=null;function k(e){O||(console.log(`[Hearly] Starting transcription recorder...`),O=new t(e,`others`,(e,t)=>{chrome.runtime.sendMessage({type:`HEARLY_TRANSCRIBE_CHUNK`,audioBase64:e,speaker:`others`,timestamp:t})}),O.start())}function A(){let e=O;e&&(e.stop(),O=null),console.log(`[Hearly] Transcription recorder stopped`)}function j(){A();let e=D;e&&(e.disconnect(),D=null);let t=T;t&&(t.close(),T=null);let n=E;n&&(n.getTracks().forEach(e=>e.stop()),E=null),console.log(`[Hearly] Audio capture stopped on Meet page`),chrome.runtime.sendMessage({type:`HEARLY_AUDIO_STOPPED`,platform:`meet`})}chrome.runtime.onMessage.addListener((e,t,n)=>{if(e.type===`HEARLY_NEW_TRANSCRIPT_ENTRY`&&e.entry&&g(e.entry),e.type===`HEARBEAT_PING`)return n({status:`pong`}),!0;e.type===`HEARLY_START_AUDIO`&&e.streamId&&navigator.mediaDevices.getUserMedia({audio:{mandatory:{chromeMediaSource:`tab`,chromeMediaSourceId:e.streamId}},video:!1}).then(e=>{if(E=e,T=new AudioContext({sampleRate:16e3}),T.state===`suspended`){let e=()=>{T?.resume(),window.removeEventListener(`click`,e)};window.addEventListener(`click`,e)}D=T.createMediaStreamSource(e),D.connect(T.destination),console.log(`[Hearly] Audio capture started on Meet page (tabCapture)`),chrome.runtime.sendMessage({type:`HEARLY_AUDIO_STARTED`,platform:`meet`}),chrome.storage.local.get(`hearly_transcript`,t=>{t.hearly_transcript?.isEnabled&&k(e)})}).catch(e=>{console.warn(`[Hearly] Meet tabCapture stream failed:`,e),chrome.runtime.sendMessage({type:`HEARLY_AUDIO_ERROR`,error:String(e)})}),e.type===`HEARLY_STOP_AUDIO`&&j(),e.type===`HEARLY_FILTER_STATE_CHANGED`&&chrome.storage.local.get(`hearly_filter`,e=>{window.postMessage({source:`hearly-content`,type:`FILTER_STATE_CHANGED`,active:e.hearly_filter?.isActive===!0},window.location.origin)}),e.type===`HEARLY_TRANSCRIPT_STATE_CHANGED`&&chrome.storage.local.get(`hearly_transcript`,e=>{let t=e.hearly_transcript?.isEnabled===!0;window.postMessage({source:`hearly-content`,type:`TRANSCRIPT_STATE_CHANGED`,enabled:t},window.location.origin),t?E&&!O&&k(E):A()})});function M(){if(document.getElementById(`hearly-indicator`))return;let e=document.createElement(`div`);e.id=`hearly-indicator`,e.style.cssText=`
    position: fixed;
    bottom: 24px;
    right: 24px;
    z-index: 999999;
