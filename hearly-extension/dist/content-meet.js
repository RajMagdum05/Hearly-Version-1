    ...
    `;t?n.innerHTML=`
      ...
      `:n.innerHTML=`
        ...
        <button id="hearly-dismiss-btn" style="
          ...
          flex-shrink: 0;
        ">×</button>
      `,document.body.appendChild(n),document.getElementById(`hearly-activate-btn`)?.addEventListener(`click`,()=>{chrome.runtime.sendMessage({type:`ACTIVATE_HEARLY`}),n.remove()}),document.getElementById(`hearly-train-btn`)?.addEventListener(`click`,()=>{chrome.runtime.sendMessage({type:`OPEN_POPUP`}),n.remove()}),document.getElementById(`hearly-dismiss-btn`)?.addEventListener(`click`,()=>{n.remove()})})}s()?(c(),u(),console.log(`Hearly: Meet room detected (lobby or active)`)):console.log(`Hearly: Meet page loaded, no active meeting`);var d=s();new MutationObserver(()=>{let e=s();e!==d&&(e?c():l(),d=e)}).observe(document.querySelector(`title`)||document.documentElement,{subtree:!0,characterData:!0,childList:!0});var f=null,p=null,m=null,h=null;function g(e){h||(console.log(`[Hearly] Starting transcription recorder...`),chrome.storage.local.get(`hearly_app_settings`,t=>{let n=t.hearly_app_settings?.language??`en`;h=new a(e,`others`,(e,t)=>{(async()=>{let r=await i(e);r&&chrome.runtime.sendMessage({type:`HEARLY_TRANSCRIBE_CHUNK`,audioBase64:e,samples:r.samples,sampleRate:r.sampleRate,language:n,speaker:`others`,timestamp:t})})()}),h.start()}))}function _(){h&&=(h.stop(),null),console.log(`[Hearly] Transcription recorder stopped`)}function v(){_(),m&&=(m.disconnect(),null),f&&=(f.close(),null),p&&=(p.getTracks().forEach(e=>e.stop()),null),console.log(`[Hearly] Audio capture stopped on Meet page`),chrome.runtime.sendMessage({type:`HEARLY_AUDIO_STOPPED`,platform:`meet`})}chrome.runtime.onMessage.addListener((e,t,n)=>{if(e.type===`HEARBEAT_PING`)return n({status:`pong`}),!0;e.type===`HEARLY_START_AUDIO`&&e.streamId&&navigator.mediaDevices.getUserMedia({audio:{mandatory:{chromeMediaSource:`tab`,chromeMediaSourceId:e.streamId}},video:!1}).then(e=>{if(p=e,f=new AudioContext({sampleRate:16e3}),f.state===`suspended`){let e=()=>{f?.resume(),window.removeEventListener(`click`,e)};window.addEventListener(`click`,e)}m=f.createMediaStreamSource(e),m.connect(f.destination),console.log(`[Hearly] Audio capture started on Meet page (tabCapture)`),chrome.runtime.sendMessage({type:`HEARLY_AUDIO_STARTED`,platform:`meet`}),chrome.storage.local.get(`hearly_transcript`,t=>{t.hearly_transcript?.isEnabled&&g(e)})}).catch(e=>{console.warn(`[Hearly] Meet tabCapture stream failed:`,e),chrome.runtime.sendMessage({type:`HEARLY_AUDIO_ERROR`,error:String(e)})}),e.type===`HEARLY_STOP_AUDIO`&&v(),e.type===`HEARLY_FILTER_STATE_CHANGED`&&chrome.storage.local.get(`hearly_filter`,e=>{window.postMessage({source:`hearly-content`,type:`FILTER_STATE_CHANGED`,active:e.hearly_filter?.isActive===!0},window.location.origin)}),e.type===`HEARLY_TRANSCRIPT_STATE_CHANGED`&&chrome.storage.local.get(`hearly_transcript`,e=>{let t=e.hearly_transcript?.isEnabled===!0;window.postMessage({source:`hearly-content`,type:`TRANSCRIPT_STATE_CHANGED`,enabled:t},window.location.origin),t?p&&!h&&g(p):_()})});