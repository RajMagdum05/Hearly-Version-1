function e(){let e=window.location.href;return e.includes(`teams.microsoft.com`)||e.includes(`teams.live.com`)}function t(){return[`[data-tid="calling-screen"]`,`[data-tid="meeting-composite"]`,`[data-tid="prejoin-screen"]`,`.ts-calling-screen`,`[class*="calling"]`,`[class*="meeting"]`].some(e=>!!document.querySelector(e))}function n(){chrome.runtime.sendMessage({type:`MEETING_DETECTED`,payload:{platform:`teams`}})}function r(){chrome.runtime.sendMessage({type:`MEETING_ENDED`})}function i(){document.getElementById(`hearly-banner`)||chrome.storage.local.get(`hearly_enrollment`,e=>{let t=e?.hearly_enrollment?.isEnrolled===!0,n=document.createElement(`div`);n.id=`hearly-banner`,n.style.cssText=`
      position: fixed;
      top: 16px;
      left: 50%;
    ...
    `;t?n.innerHTML=`
      ...
      `:n.innerHTML=`
        ...
        <button id="hearly-dismiss-btn" style="
          ...
          flex-shrink: 0;
        ">×</button>
      `,document.body.appendChild(n),document.getElementById(`hearly-activate-btn`)?.addEventListener(`click`,()=>{chrome.runtime.sendMessage({type:`ACTIVATE_HEARLY`}),n.remove()}),document.getElementById(`hearly-train-btn`)?.addEventListener(`click`,()=>{chrome.runtime.sendMessage({type:`OPEN_POPUP`}),n.remove()}),document.getElementById(`hearly-dismiss-btn`)?.addEventListener(`click`,()=>{n.remove()})})}function a(){if(!e())return;console.log(`[Hearly] Teams domain detected — watching for meeting...`),i(),t()&&(n(),console.log(`[Hearly] Teams meeting active on load`));let a=t();new MutationObserver(()=>{let e=t();e!==a&&(e?(n(),console.log(`[Hearly] Teams meeting started`)):(r(),console.log(`[Hearly] Teams meeting ended`)),a=e),document.getElementById(`hearly-banner`)||i()}).observe(document.body,{subtree:!0,childList:!0});let o=window.location.href;setInterval(()=>{window.location.href!==o&&(o=window.location.href,console.log(`[Hearly] Teams URL changed:`,o),document.getElementById(`hearly-banner`)||i())},1e3)}a();var o=null,s=null,c=null;function l(){c&&=(c.disconnect(),null),o&&=(o.close(),null),s&&=(s.getTracks().forEach(e=>e.stop()),null),console.log(`[Hearly] Audio capture stopped on Teams page`),chrome.runtime.sendMessage({type:`HEARLY_AUDIO_STOPPED`,platform:`teams`})}chrome.runtime.onMessage.addListener(e=>{e.type===`HEARLY_START_AUDIO`&&e.streamId&&navigator.mediaDevices.getUserMedia({audio:{mandatory:{chromeMediaSource:`tab`,chromeMediaSourceId:e.streamId}},video:!1}).then(e=>{s=e,o=new AudioContext({sampleRate:16e3}),c=o.createMediaStreamSource(e),console.log(`[Hearly] Audio capture started on Teams page (tabCapture)`),chrome.runtime.sendMessage({type:`HEARLY_AUDIO_STARTED`,platform:`teams`})}).catch(e=>{console.warn(`[Hearly] Teams tabCapture stream failed:`,e),chrome.runtime.sendMessage({type:`HEARLY_AUDIO_ERROR`,error:String(e)})}),e.type===`HEARLY_STOP_AUDIO`&&l()});